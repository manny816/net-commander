...
(function () {
  ...
  function initTimeChart(id, yDomain) {
    ...
    const reference = g.append('line')
      .attr('class', 'reference-line')
      .attr('x1', 0)
      .attr('x2', innerWidth)
      .attr('stroke-dasharray', '5,5');

    return { g, x, y, line, path, reference, innerWidth };
  }
  ...
  function renderTimeChart(chart, points, referenceValue) {
    ...
    if (Number.isFinite(referenceValue)) {
      chart.reference
        .attr('y1', chart.y(referenceValue))
        .attr('y2', chart.y(referenceValue))
        .attr('stroke', 'var(--vscode-descriptionForeground)');
    }
  }
  ...
  function updateSummary(info) {
    ...
    setText('bssid-val', info.bssid || 'Pending BSSID resolver');
  ...
  function updateRawEvidence(info) {
    ...
    const fields = [
      ...
      ['Channel width', Number.isFinite(info.widthMHz) ? `${info.widthMHz} MHz` : undefined],
      ...
      ['MCS index', info.mcsIndex],
      ['Security', info.security],
      ['Network type', info.networkType],
      ['RX bytes', info.rxBytes],
      ['TX bytes', info.txBytes],
      ['Dropped RX', info.rxDropped],
      ['Dropped TX', info.txDropped],
    ];
  ...
  function neighborStatus(info) {
    const neighborPanel = document.querySelector('#neighbor-signal-block svg');
    if (!neighborPanel) return;

    if (Array.isArray(info.neighborDetails) && info.neighborDetails.length) {
      neighborPanel.outerHTML = '<div class="coming-data">Neighbor telemetry available. CCI/ACI scoring is the next correlation layer.</div>';
    } else if (neighborPanel.tagName.toLowerCase() === 'svg') {
      const holder = document.createElement('div');
      holder.className = 'coming-data';
      holder.textContent = 'Neighbor BSSID/channel scan pending. Current-link RF evidence is authoritative; interference scoring is not yet available.';
      neighborPanel.replaceWith(holder);
    }

    document.querySelectorAll('.footer svg').forEach(svg => {
      if (svg.dataset.placeholderDone) return;
      const holder = document.createElement('div');
      holder.className = 'coming-data';
      holder.textContent = 'Channel occupancy / overlap evidence pending neighbor collector.';
      svg.dataset.placeholderDone = '1';
